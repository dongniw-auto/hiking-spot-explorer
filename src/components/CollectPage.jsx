...
export default function CollectPage({ bounties, completedTaskIds, completeTask, totalStardust, completions }) {
        ...
        <section className="collect-section">
          ...
          <div className="collect-bounty-groups">
            {Object.entries(grouped).map(([spotId, group]) => (
              <div key={spotId} className="collect-spot-group">
      ...
      {completed.length > 0 && (
        <section className="collect-section">
          ...
          <div className="collect-completed-list">
            {completed.map((task) => {
              const record = completions.find((c) => c.taskId === task.id)
              return (
                <div key={task.id} className="collect-completed-card">
                  ...
                  <div className="collect-completed-info">
                    ...
                    <div className="collect-completed-spot">
                      {task.spotName}{record ? ` · ${timeAgo(record.completedAt)}` : ''}
                    </div>
                  </div>
